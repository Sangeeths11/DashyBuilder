<template>
  <div class="w-full h-full flex flex-col justify-between">
    ...
    <div class="bg-white shadow-lg rounded-lg p-4 h-full flex flex-col relative">
      <h2 class="font-bold text-lg mb-4">Upload Mockup</h2>
      <div class="flex flex-col items-center flex-grow w-full">
        <label for="file-upload"
          class="w-full flex flex-col items-center justify-center px-4 py-20 bg-gray-100 text-blue rounded-lg shadow-lg tracking-wide uppercase border border-gray-200 cursor-pointer hover:bg-blue-500 hover:text-white transition duration-300 ease-in-out mb-6">
          <svg class="w-16 h-16" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            <path
              d="M16.88 5.5a2 2 0 01.12 0h1a2 2 0 011.72 1.05 2 2 0 01.28.96v7a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h1.1a2 2 0 01.12 0H5a.5.5 0 000 1h10a.5.5 0 000-1h-.12zM9 8a.5.5 0 000 1h2a.5.5 0 000-1H9z" />
          </svg>
          <span class="mt-3 text-lg font-semibold leading-normal">SELECT A FILE</span>
          <span class="text-sm leading-normal mt-1">File must be in PNG format</span>
          <input id="file-upload" type="file" class="hidden" @change="handleFileUpload" />
        </label>
        <div class="w-full mt-auto">
          <button @click="uploadDataset" :disabled="!selectedFile || isLoading"
            ...
            <span v-else>Upload Mockup</span>
          </button>
        </div>
      </div>
    </div>
...
const uploadDataset = async () => {
  ...
  try {
    ...
    successMessage.value = 'Mockup uploaded successfully';
...
onMounted(async () => {
  uploadedMockupId.value = await projectStore.fetchProjectFilePath(props.projectId);
});
</script>
